    ...
    public static class EntitySpawner
    {
        ...
        public static void Initialize()
        {
            lock (_initLock)
            {
                ...
                {
                    Plugin.Log.LogError($"[EntitySpawner] Initialization failed: {ex}");
                }
            }
        }
        ...
        public static SpawnResult SpawnGlowingBuffEntities(SpawnConfig? config = null)
        {
            ...
            {
                Plugin.Log.LogError($"[EntitySpawner] Batch spawn failed: {ex}");
            ...
            ref Random random)
        {
            ...
            {
                Plugin.Log.LogError($"[EntitySpawner] Failed to create entity at {position}: {ex}");
                return Entity.Null;
            }
        }

        /// <summary>
        /// Apply a buff to an entity using Buffs utility.
        /// </summary>
        private static void ApplyBuffToEntity(Entity entity, int buffId)
        {
            try
            {
                ...
                if (Buffs.AddBuff(entity, entity, buffGuid, 30f, immortal: false))
                {
                    Plugin.Log.LogDebug($"[EntitySpawner] Applied buff {buffId} to entity");
                    return;
                }

                Plugin.Log.LogWarning($"[EntitySpawner] Failed applying buff {buffId}");
            }
            catch (Exception ex)
            {
                Plugin.Log.LogWarning($"[EntitySpawner] Failed to apply buff {buffId}: {ex}");
            }
        }
        ...
        public static void DespawnAll(NativeList<Entity> entities)
        {
            ...
            {
                Plugin.Log.LogError($"[EntitySpawner] Failed to despawn entities: {ex}");
            }
        }
        ...
        public static void UpdateGlowConfig(NativeList<Entity> entities, GlowConfig newConfig)
        {
            ...
            {
                Plugin.Log.LogError($"[EntitySpawner] Failed to update glow config: {ex}");
            }
        }
