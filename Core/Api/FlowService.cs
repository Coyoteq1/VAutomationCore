...
namespace VAutomationCore.Core.Api
{
    ...
    public static class FlowService
    {
        ...
        private static readonly ConcurrentDictionary<string, string> FlowModuleOwners = new(StringComparer.OrdinalIgnoreCase);
            ...
            lock (ModuleFlowSync)
            {
                ...
                foreach (var pair in flows)
                {
                    ...
                    if (Flows.ContainsKey(flowName) &&
                        FlowModuleOwners.TryGetValue(flowName, out var owner) &&
                ...
                foreach (var pair in flows)
                {
                    ...
                    FlowModuleOwners[flowName] = moduleKey;
        ...
        private static bool UnregisterModuleFlowsInternal(string moduleId)
        {
            ...
            {
                if (FlowModuleOwners.TryGetValue(flowName, out var owner) &&
                    string.Equals(owner, moduleId, StringComparison.OrdinalIgnoreCase))
                {
                    Flows.TryRemove(flowName, out _);
                    FlowModuleOwners.TryRemove(flowName, out _);
                }
            }
        ...
        private static void DetachFlowOwnership(string flowName)
        {
            ...
            if (!FlowModuleOwners.TryRemove(flowName, out var owner) || string.IsNullOrWhiteSpace(owner))
            {
                return;
            }
