    ...
    public static class TypedJsonConfigManager
    {
            ...
            Action<string>? logError = null)
        {
            ...
            catch (Exception ex)
            {
                createdDefault = true;
                config = defaultFactory();
                logError?.Invoke($"Failed to load config '{path}': {ex}. Reverting to defaults.");
                TrySave(path, config, options, logInfo, logError);
                return false;
            }
        }
            ...
            Action<string>? logError = null)
        {
            ...
            {
                logError?.Invoke($"Failed to save config '{path}': {ex}");
                return false;
            }
        }
        ...
        private static void EnsureDirectory(string path, Action<string>? logError)
        {
            ...
            {
                logError?.Invoke($"Failed to create config directory for '{path}': {ex}");
            }
        }
    }
}
