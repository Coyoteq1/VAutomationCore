    ...
    public class HttpServer : IDisposable
    {
        ...
        public async Task StartAsync()
        {
            ...
            while (!_cts.Token.IsCancellationRequested)
            {
                ...
                catch (Exception ex) { Plugin.LogInstance.LogWarning($"[HttpServer] Error: {ex}"); }
            }
        }
        ...
        private async Task HandleRequestAsync(HttpListenerContext context)
        {
            ...
            {
                Plugin.LogInstance.LogError($"[HttpServer] Request error: {ex}");
        ...
        private static (int offset, int limit) ParsePagination(string query)
        {
            ...
            {
                Debug.Log($"[HttpServer] Query string parse error (using defaults): {ex}");
            }
            return (offset, limit);
        }
        ...
        private async Task HandleStatusAsync(HttpListenerContext context)
        {
            try
            {
                var status = new StatusDto
                {
                    ...
                    ZonesActive = 0,
                    Plugins = new[] { new PluginStatusDto { Name = "Blueluck", Version = VAutomationCore.Core.MyPluginInfo.VERSION, Enabled = true, Status = "active" } }
                };
